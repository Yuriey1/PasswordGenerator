...
├── Reset-DomainPasswords-ByEmail.ps1 # Дополнительный скрипт поиска учетки по имени в email (это если не нашли по ФИО, то по логину точно найдет, доказано опытным путем)
...
**Пример:**
```
Сидоров Сидор Сидорович (sss001)
```
...
4. **Поиск в AD** — поиск пользователей по DisplayName (ФИО) или по логину, если быз запущен второй скрипт
