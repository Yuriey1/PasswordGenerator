...
if (-not $SkipInfisical) {
    ...
    # Get search order from config (or use all keys)
    if ($ic.EnvironmentOrder -and $ic.EnvironmentOrder.Count -gt 0) {
        $envOrder = $ic.EnvironmentOrder
    } else {
        # Fallback: use all keys from map (order not guaranteed)
        $envOrder = @($envMap.Keys)
    }
    
    Write-Host ""
    Write-Host "  Environment mapping:" -ForegroundColor Cyan
    foreach ($key in $envOrder) {
        $slug = $envMap[$key]
        if ($slug) {
            Write-Host "    '$key' -> '$slug'" -ForegroundColor Gray
        }
    }
    Write-Host ""
    Write-Host "  Search order: $($envOrder -join ' -> ')" -ForegroundColor Cyan
}
...
foreach ($u in $script:NormalizedUsers) {
    ...
    $domain = $null
    if ($email -match "^([^@]+)@(.+)$") {
        $login = $matches[1].ToLower()  # Normalize login to lowercase
    ...
    # Check if password already exists in Infisical (in ANY environment, in specified order)
